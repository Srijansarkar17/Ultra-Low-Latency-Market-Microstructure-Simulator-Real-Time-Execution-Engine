...
async def recv_once(): # A coroutine that will run asynchronously (non-blocking).
    async with websockets.connect(WS_URL, ping_interval=15, ping_timeout=10) as ws: # Opens the WebSocket connection to Binance. , pinginterval means sending an intenval every 15 seconds, ping_timeout means if Binance doesn't respond within 10 seconds, the connection closes.
        ...
        while True: #Loop forever to continuously recieve incoming messages
            ...
            now_us = int(time.time() * 1_000_000) # Capture the local time (in milliseconds) at the exact moment you received the message, Useful for latency calculations and logging.
            ...
            evt_us = int(data.get("E", int(time.time() * 1_000_000))) # Binance includes a field "E" inside data → this is the timestamp (in milliseconds) when Binance created the event. 
            ...
            latency_ms = (now_us - evt_us) / 1000.0 # now_ms = when YOU received the message, evt_ms = when BINANCE created the message, latency_ms = how many milliseconds the message took to reach you.
